...
void intCandLine::modifyNodeReact() // function to modify the nodal connected reactance, if the candidate line is actually built
{
	int fromNode=connNodetPtr1->getNodeID();
	int toNode=connNodetPtr2->getNodeID();
	connNodetPtr1->modifyReactAPP( translID, 1, reacT, toNode, 0 ); // increments the txr line connection variable to node 1
	connNodetPtr2->modifyReactAPP( translID, -1, reacT, fromNode, 0 ); // increments the txr line connection variable to node 1
} // function ends
...
void intCandLine::setPresAbsStatus() // Sets the construction status of the candidate line
{
	resFromStageI=1; 
}
