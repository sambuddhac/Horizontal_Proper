...
############################################################################################/* Generators */
    ...
    ######################################################################################/* Internal Candidate Transmission Lines */
        ...
        k1 = 0 #Counter for internal candidate lines
		...
		for matrixIntCETran in matrixIntCETranFile:
			...
			intCandLineInstance = intCandLine( k1 + 1, self.nodeObject[ tNodeID1 - 1 ], self.nodeObject[ tNodeID2 - 1 ], ptMax, reacT, interestRate, lifeTime, costPerCap, presAbsence ) #Create the internal candidate transmission line object with node 1 
			self.intCandLineObject.append( intCandLineInstance ) #pushes the transLineInstance object into the vector
            k2 +=1 #increment the counter
...
############################################################################################ Can't underestand what are these codes doing
...
###################################################################################################### 
""" I don't know what are the following codes doing...
	}
...
########################################################################################### Shared Existing Transmission lines
		...
		for matrixSETran in matrixSETranFile:
			...
			SELineInstance = SELine(H1 + 1, serNum, self.nodeObject[ tNodeID1 - 1 ], tNodeID1, nodeZone1, tNodeID2, nodeZone2, self.zonalIndex, ptMax, reacT) #Create the shared existing transmission line object with node 1 
	...
	""" I didn't translate the following codes because I can't get what are they going to do????
