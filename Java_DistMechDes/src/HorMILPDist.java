...
public class HorMILPDist {
	...
	public static void main(String[] args) // Main method begins program execution
	{
		...
		int basisForComparison = 0; // While updating the Lagrange multipliers, 0 when the current zonal updates are compared to previous iteration MO update;else, 1
		int solverChoice =  0; // Choice of the solver
		...
		System.out.println("\nChoose, for updating the Lagrange multipliers, 0 when the current zonal updates are compared to previous iteration MO update;else, 1");
		Scanner compBasisInput = new Scanner(System.in);
		basisForComparison = compBasisInput.nextInt();
		...
		int lpMethodChoice = 0; // Simplex or interior method algorithm for MILP
		...
		System.out.println("\nChoose either the GLPK (1) or GUROBI (2) as the Solver. ");
		Scanner solChoiceInput = new Scanner(System.in);
		solverChoice = solChoiceInput.nextInt();
		if (solverChoice == 1) {
			System.out.println("\nChoose either the Simplex LP Rlaxation (1) or Interior Point Method LP Relaxation (2) as the method to provide the initial basis to the Mixed Integer Unit Commitment Problem. ");
			Scanner lpmethodChoiceInput = new Scanner(System.in);
			lpMethodChoice = lpmethodChoiceInput.nextInt();
		}
		...
		Vector<Nettran>zonalNetVector = new Vector<Nettran>(); // Vector of zonal network objects
		...
		for (int i = 0; i < numberOfZones; ++i) {
			...
			zonalNetVector.add(nettranInstance); // push to the vector of zonal networks
		}
	...
	*/
		...
		Vector<Integer> sharedNodeList; // List of all the node IDs of the shared existing and/or shared candidate transmission lines
		Vector<Integer> sharedZoneList; // List of all the corresponding zone IDs of the respective shared line end nodes
		Vector<Integer> sharedGlobalNodeList; // Global ranking of the shared nodes
		sharedNodeList.add(0); // Initialize the list of shared line end node ID's so that it's not empty
		sharedZoneList.add(0); // Initialize the list of shared line end zone ID's so that it's not empty
		sharedGlobalNodeList.add(0); // Initialize the list of shared global line end zone ID's so that it's not empty
		...
		Vector<Integer> sharedSESerList; // List of all the unique global serial numbers of the shared existing lines
		sharedSESerList.push_back(0); // Initialize the list of shared existing line global serial numbers so that it's not empty
		Vector<Integer> sharedSEFromList; // List of the From node ranks of the shared existing lines
		Vector<Integer> sharedSEToList; // List of the To node ranks of the shared existing lines
		...
		Vector<Integer> sharedCandSerList; // List of all the unique global serial numbers of the shared candidate lines
		sharedCandSerList.push_back(0); // Initialize the list of shared candidate line global serial numbers so that it's not empty
		Vector<Integer> sharedCandFromList; // List of the From node ranks of the shared candidate lines
		Vector<Integer> sharedCandToList; // List of the To node ranks of the shared candidate lines
		...
		for (int i = 0; i < numberOfZones; ++i) { // Run the loop on all the zones
			...
			for (int sharedNodeCounter = 1; ((sharedZone != -1) && (sharedNode != -1)); ++sharedNodeCounter) { // Run the loop on shared lists
				sharedZone = zonalNetVector[i]->
				getConnZone(sharedNodeCounter); // Gets the zone ID of the outer zonal node
				sharedNode = zonalNetVector[i]->
				getConnNode(sharedNodeCounter); // Gets the node ID of the outer zonal node
				...
				if ((sharedZone != -1) && (sharedNode != -1)) { // as long as end of the zonal vectors are not reached
					for (diffZNIt = sharedNodeList.begin(); diffZNIt != sharedNodeList.end(); ++diffZNIt) { // Iterate over the vector of connected external zone nodes
						if (( * diffZNIt == sharedNode) &&(sharedZoneList[indCount] == sharedZone))
						{ // Check whether the other-zone node is already present in the list
					...
					for (SESerList = sharedSESerList.begin(); SESerList != sharedSESerList.end(); ++SESerList) { // Iterate over the list of SELine
						if (*SESerList == sharedELine){ // Check whether the SE line is already present in the list
							...
							for (diffZNIt = sharedNodeList.begin(); diffZNIt != sharedNodeList.end(); ++diffZNIt) {
								if (( * diffZNIt == SEFromNode) &&(sharedZoneList[indCount] == SEFromZone))
								{ // Check whether the from node is already present in the list
								...
								if (( * diffZNIt == SEToNode) &&(sharedZoneList[indCount] == SEToZone))
								{ // Check whether the to node is already present in the list
			...
			for (int candLineCounter = 0; (sharedCandLine != -1); ++candLineCounter) { // Run the loop on shared candidate lines
				...
				if (sharedCandLine != -1) { // as long as end of the zonal candidate line vector is not reached
					...
					for (CandSerList = sharedCandSerList.begin(); CandSerList != sharedCandSerList.end(); ++CandSerList) { // Iterate over candline
						if (*CandSerList == sharedCandLine)
						{ // Check whether the Cand line is already present in the list
							...
							int indCount = 0; // Initialize a counter for tracking the position in the vector of the iterator
							for (diffZNIt = sharedNodeList.begin(); diffZNIt != sharedNodeList.end(); ++diffZNIt) {
								if (( * diffZNIt == CandFromNode) &&(sharedZoneList[indCount] == CandFromZone))
								{ // Check whether the from node is already present in the list
								...
								if (( * diffZNIt == CandToNode) &&(sharedZoneList[indCount] == CandToZone))
								{ // Check whether the to node is already present in the list
					...
					if (containsCandFlag == 0) { // If the diffZoneNodeID and diffZoneID vectors do not contain the other zone-node, then push the node and the other zone index in the respective vectors
						...
						for (diffZNIt = sharedNodeList.begin(); diffZNIt != sharedNodeList.end(); ++diffZNIt) {
							...
							{ // Check whether the from node is already present in the list
								...
								//System.out.println("From node of candidate line " << sharedCandLine << " is " << CandFromNode << " From zone is " << CandFromZone << " and from rank is " << CandFromRank << endl;
							}
							if (( * diffZNIt == CandToNode) &&(sharedZoneList[indCount] == CandToZone))
							{ // Check whether the to node is already present in the list
								CandToRank = indCount; // if yes, get the rank of the to node from the list (incrementing indCount will point to the rank, since sharedNodeList and sharedZoneList both have a dummy 0 as their fist element
								...
								//System.out.println("To node of candidate line " << sharedCandLine << " is " << CandToNode << " To zone is " << CandToZone << " and to rank is " << CandToRank << endl;
							}
							++indCount; // Increment the counter
						}
		...
		double LagMultXi[ (scenarios + 1) * numberOfZones * otherNodeCount]
		; // Lagrange Multipliers/Dual Variables/Rewards/Penalties corresponding to bus voltage phase angle consensus
		double LagMultPi[ numberOfZones * candLineCount]
		; // Lagrange Multipliers/Dual Variables/Rewards/Penalties corresponding to line building decision integer variable consensus
		double LagCombXi[ (scenarios + 1) * otherNodeCount]
		; // Lagrange Multipliers/Dual Variables/Rewards/Penalties corresponding to bus voltage phase angle consensus for MO
		double LagCombPi[ candLineCount]
		; // Lagrange Multipliers/Dual Variables/Rewards/Penalties corresponding to line building decision integer variable consensus MO
		...
		ofstream UBIteratesOut
		("/home/samie/code/Horizontal_Coordination_MILP_Dist_Mechanism_Design/Horizontal_Proper/output/outputMetrics/UpperBoundIterates.txt", ios::out)
		;
		// exit program if ifstream could not open file
		if (!UBIteratesOut) {
		...
		ofstream LBIteratesOut
		("/home/samie/code/Horizontal_Coordination_MILP_Dist_Mechanism_Design/Horizontal_Proper/output/outputMetrics/LowerBoundIterates.txt", ios::out)
		;
		// exit program if ifstream could not open file
		if (!LBIteratesOut) {
		...
		ofstream ratioIteratesOut
		("/home/samie/code/Horizontal_Coordination_MILP_Dist_Mechanism_Design/Horizontal_Proper/output/outputMetrics/RatioIterates.txt", ios::out)
		;
		// exit program if ifstream could not open file
		if (!ratioIteratesOut) {
		...
		ofstream globalConsensusOut
		("/home/samie/code/Horizontal_Coordination_MILP_Dist_Mechanism_Design/Horizontal_Proper/output/outputMetrics/globalConsensus.txt", ios::out)
		;
		// exit program if ifstream could not open file
		...
		for (int i = 0; i < numberOfZones; ++i) { // Each region solves its own MILP optimization problem
			zonalNetVector[i]->setRealizedCLines( * marketoverInstance)
			; // Perform unit commitment for average heat rate objective
